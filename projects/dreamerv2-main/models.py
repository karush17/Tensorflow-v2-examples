...
class WorldModel(tools.Module):
  ...
  def train(self, data):
    ...
    with tf.GradientTape() as model_tape:
      ...
      # GET PRIOR AND POSTERIOR UPON ACTION
      ...
      for name, head in self.heads.items():
        # STOP/ALLOW GRADS FOR EACH HEAD
...
class ImagBehavior(tools.Module):
  ...
  def train(
    ...
    with (tape or tf.GradientTape()) as actor_tape:
      ...
      # LEARN FROM WORLD MODEL TO UPDATE OBJECTIVE
      if objective:
        imag_feat, imag_state, imag_action = self._imagine(
            start, self.actor, self._config.imag_horizon, repeats)
        reward = objective(imag_feat, imag_state, imag_action)
      # PERFORM TASK IN ENVIRONMENT
      else:
    ...
    # UPDATE ACTOR AND VALUE OPTIMIZERS
  ...
  def _imagine(self, start, policy, horizon, repeats=None):
    ...
    # UNFOLD FEATURES, STATES AND ACTIONS FOR ACTION REPEAT
    if repeats:
      def unfold(tensor):
        s = tensor.shape
  ...
  def _compute_actor_loss(
    ...
    if self._config.imag_gradient == 'dynamics':
      actor_target = target
    # USE REINFORCE FOR IMAGE GRADIENTS
      ...
      actor_target = policy.log_prob(imag_action)[:-1] * tf.stop_gradient(
          target - self.value(imag_feat[:-1], tf.float32).mode())
      # MIX IMAGE GRADIENTS BETWEEN REINFORCE AND DYNAMICS
